...
- (UIImage *)cropImage:(CGRect)rect
{
    /*UIGraphicsBeginImageContext(rect.size);
     ...
     CGContextClipToRect(contexto, CGRectMake(0.0, 0.0, rect.size.width, rect.size.height));
     [self drawInRect:CGRectMake(-rect.origin.x, -rect.origin.y, self.size.width, self.size.height)];
     UIImage *imagedCroped = UIGraphicsGetImageFromCurrentImageContext();
     UIGraphicsEndImageContext();
     return imagedCroped;*/
...
{
    return [self applyBlurWithRadius:50 tintColor:[UIColor colorWithWhite:1 alpha:0.2] saturationDeltaFactor:1.5 maskImage:nil];
}

- (UIImage *)blurWithCoreImage:(UIImage *)sourceImage atView:(UIView *)view
{
